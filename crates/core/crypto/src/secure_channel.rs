...
impl SecureChannel {
    ...
    pub fn encrypt(
        ...
        Ok(securemessage::SecureMessage {
            header_and_body,
    ...
    fn decode_payload(&mut self, plain: &[u8]) -> Result<Vec<u8>, CryptoError> {
        let packet = securegcm::DeviceToDeviceMessage::decode(plain)
            .map_err(|_| CryptoError::InvalidMessage)?;
        let expected = self
            .receive_sequence
            .checked_add(1)
        ...
        packet.message.ok_or(CryptoError::InvalidMessage)
    }
}
...
fn validate_header(header: &securemessage::Header) -> Result<(), CryptoError> {
    ...
    let metadata = securegcm::GcmMetadata::decode(
        header
            .public_metadata
            .as_deref()
            .ok_or(CryptoError::InvalidMessage)?,
