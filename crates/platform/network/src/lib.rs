//! Linux DNS-SD and TCP support for Quick Share media.
...
/// A DNS-SD adapter failure.
#[derive(Clone, Debug, Eq, PartialEq)]
#[expect(
    clippy::error_impl_error,
    reason = "The public DNS-SD error keeps the established adapter error name"
)]
...
#[expect(
    ...
    reason = "The DNS-SD error stores display text and has no source error"
)]
...
/// Converts an mDNS daemon error into the adapter error type.
fn convert_error(error: &mdns_sd::Error) -> Error {
    Error(error.to_string())
}

/// Converts a daemon service record into the public DNS-SD result.
#[expect(
