...
Live Sharing interoperability uses a pinned, test-only control wrapper around the Apache-2.0 Linux Nearby fork's `nearby_sharing_cli`. Pinned NearShare computes its PIN fingerprint with Python's nonnegative `% 9973`, unlike Google's C++ signed remainder, so the prepared-source adapter emulates the C++ arithmetic without changing immutable peer pins. Its evidence parser accepts ordinary JSON whitespace. `make test-diverse-lan` passed all three direction/repeat cases in 22.56 seconds with matching PIN and exact bytes. This proves the adapted peer and Wi-Fi LAN lab, not stock NearShare, Bluetooth, hotspot, or Wi-Fi Direct compatibility.
