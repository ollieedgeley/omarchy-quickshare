...
Live diverse-peer testing uses a pinned, test-only `prepareNearShareSource` adapter that rewrites only `nearshare/core/crypto.py::pin_code` so Python emulates Google's C++ signed `% 9973`; immutable NearShare pins and source remain unchanged, and the evidence parser recognizes NearShare's existing JSON fingerprint. `make test-diverse-lan` passed in 22.56 seconds including lifecycle, with equal per-run salted PIN fingerprints and exact FILE bytes for Google-to-NearShare, NearShare-to-Google, and a clean repeated Google-to-NearShare transfer. This proves the prepared isolated adapter/reference path, not stock NearShare, Android, or other transports.
